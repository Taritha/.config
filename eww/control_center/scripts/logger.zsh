...
function make_literal() {
  ...
  [[ "$caches" == "" ]] \
    && print '(box :class "control-center-empty-box" :height 510 :vexpand true :orientation "vertical" :space-evenly false (image :class "control-center-empty-banner" :valign "end" :vexpand true :path "" :image-width 200 :image-height 200) (label :vexpand true :valign "start" :wrap true :class "control-center-empty-label" :text "'$quote'"))' \
    || print "(scroll :height 510 :vexpand true :vscroll true (box :orientation 'vertical' :class 'control-center-scroll-box' :spacing 15 :space-evenly false $caches))"
}
