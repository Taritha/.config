...
def handle_keys(brightlevel):
    ...
    with Events() as events:
        while True:
            ...
            # If nothing is pressed for 1.0s, stop
            if event is None:
            ...
            if str(event.key) == '<269025026>':
                # Ignore release events
                if isinstance(event, Events.Release): continue
...
if __name__ == '__main__':
    ...
    with pidfile.PIDFile():
        query_brightness = 'python /home/taritha/.config/eww/control_center/scripts/get_brightnesslevel.py'
        cmd_output = run(query_brightness.split(), cwd='/', capture_output=True)
        ...
        # Tacks brightness levels to elminiate need for re-querying
